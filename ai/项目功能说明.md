...
├── prompts/             # 提示词
│   └── prompt.py
├── .agentz/             # 会话数据目录（历史记录、摘要、meta）
│   └── sessions/
│       └── <session_id>/
│           ├── agentz_history
│           ├── agentz_message_history.json
│           ├── session_meta.json
...
```bash
# AgentZ 数据根目录通过启动参数 --agentz-home 或进程环境变量 AGENTZ_HOME 指定；默认 ~/.agentz
...
会话数据默认保存在 `AGENTZ_HOME/sessions/<session_id>/`。如果未配置 `AGENTZ_HOME`，默认使用 `~/.agentz`。
运行时环境变量从 `AGENTZ_HOME/.env` 读取；未配置 `SKILLS_DIR` 与 `MCP_CONFIG_PATH` 时，分别使用 `AGENTZ_HOME/skills` 与 `AGENTZ_HOME/mcp.json`。
